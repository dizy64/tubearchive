...
### 파이프라인 흐름 (app/cli/main.py:run_pipeline)
```
...
**app/cli/main.py**: CLI 인터페이스 및 파이프라인 오케스트레이터
...
- `detect_metadata()`: ffprobe 서브프로세스 → `VideoMetadata` 반환 (해상도/코덱/fps/픽셀포맷/색공간/길이/비트레이트)
...
**domain/media/detector.py**: 기기 모델 감지 보조
...
- `models.py`: TUI 전용 상태 모델 (`PipelineState`, `UploadState` 등)
- `screens/pipeline.py`: 파이프라인 실행 화면
