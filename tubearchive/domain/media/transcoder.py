...
라우드니스 정규화는 트랜스코딩 단계가 아닌 병합 직후 단계
(:func:`tubearchive.app.cli.pipeline._apply_post_merge_loudnorm`)에서
전체 영상에 1회만 적용한다. 클립별 평탄화를 피해 클립 간 상대 라우드니스를
보존하면서 전체 평균만 타깃(LOUDNORM_TARGET_I)에 맞춘다.
"""
...
from tubearchive.infra.ffmpeg.profiles import PROFILE_SDR, EncodingProfile, get_fallback_profile
    ...
    def transcode_video(
        ...
        if trim_silence and not has_processing_audio:
            ...
            try:
                ...
                if silence_segments:
                    from tubearchive.infra.ffmpeg.effects import create_silence_remove_filter
