...
:class:`~tubearchive.domain.models.video.VideoFile` 리스트로 반환한다.
...
def _warn_if_slow_network_source(remote_source_root: Path) -> None:
    """원격/외장 스토리지 처리 경고."""
    logger.info("원격/외장 스토리지 감지: %s", remote_source_root)

    try:
        _check_remote_source(remote_source_root)
    except OSError as exc:
        logger.warning("원격/외장 스토리지 접근 확인 실패: %s (속도 검사 생략)", exc)
        return

    speed_bps = _measure_source_read_speed(remote_source_root)
    if speed_bps is None:
        logger.warning(
            "원격/외장 스토리지에서 속도 측정에 실패했습니다: %s",
            remote_source_root,
        )
        return
