...
def test_generate_subtitles_rejects_invalid_model(tmp_path: Path) -> None:
    """지원하지 않는 모델은 SubtitleGenerationError."""
    video = tmp_path / "video.mp4"
    video.touch()

    with pytest.raises(SubtitleGenerationError, match="Unsupported subtitle model"):
        generate_subtitles(video, model="invalid")


def test_generate_subtitles_raises_for_missing_video(tmp_path: Path) -> None:
    """없는 동영상 경로는 SubtitleGenerationError."""
    with pytest.raises(SubtitleGenerationError, match="Video file not found"):
